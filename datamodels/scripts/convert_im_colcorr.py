...
# 15 Oct 2014: Minor update to setting the required keywords, for CDP-3.
#
...
Script `convert_im_colcorr` creates a FITS file in standard MIRI format 
from an input ASCII table. The script relies on the 
MiriImagingColourCorrectionModel data product, which is based on the STScI 
data model. 
...
The following command arguments are defined by position:
    ...
    inputtype[0]
        The type of flux conversion file. Valid values are IMCOLCORR.
    inputfile[1]
...
def load_colcorr_file( filename, metafile ):
    """
    ...
    colcorrdata = np.array(ascii.read(filename, data_start=0, delimiter='\s', \
        names=MiriImagingColourCorrectionModel.fieldnames))
...
if __name__ == "__main__":
    ...
    usage += "Converts an flux conversion file into "
    ...
    if inputtype == 'IMCOLCORR':
        metadata, inputdata = load_colcorr_file( inputfile, metafile )

    with MiriImagingColourCorrectionModel( flux_table=inputdata ) as colcorrproduct:
        # default modifications for CDP-1 specifications
        colcorrproduct.meta.filename_original = os.path.basename(inputfile)
        colcorrproduct.meta.filename = os.path.basename(outputfile)

        # Based on inputtype, set required keywords
        if inputtype == 'IMCOLCORR' and not metadata is None:
            colcorrproduct.meta.instrument.name = 'MIRI'
            colcorrproduct.meta.instrument.model = set_meta_key( colcorrproduct.meta.instrument.model, metadata, 'MODELNAM' )
            colcorrproduct.meta.instrument.detector = set_meta_key( colcorrproduct.meta.instrument.detector, metadata, 'DETECTOR' )
            colcorrproduct.meta.instrument.detector_settings = set_meta_key( colcorrproduct.meta.instrument.detector_settings, metadata, 'DETSETNG' )
            colcorrproduct.meta.exposure.readpatt = set_meta_key( colcorrproduct.meta.exposure.readpatt, metadata, 'READPATT' )
            colcorrproduct.meta.subarray.name = set_meta_key( colcorrproduct.meta.subarray.name, metadata, 'SUBARRAY' )
            colcorrproduct.meta.instrument.filter = set_meta_key( colcorrproduct.meta.instrument.filter, metadata, 'FILTER' )
            colcorrproduct.meta.version = set_meta_key( colcorrproduct.meta.version, metadata, 'VERSION' )
            colcorrproduct.meta.author = set_meta_key( colcorrproduct.meta.author, metadata, 'AUTHOR' )
            colcorrproduct.meta.origin = set_meta_key( colcorrproduct.meta.origin, metadata, 'ORIGIN' )
            colcorrproduct.meta.description = set_meta_key( colcorrproduct.meta.description, metadata, 'DESCRIP' )

        if verb:
            print(colcorrproduct)
        if makeplot:
            colcorrproduct.plot()

        colcorrproduct.save( outputfile, overwrite=overwrite)
        if verb:
