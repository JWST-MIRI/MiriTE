...
in the datamodels.miri_pixel_saturation_model module.
...
from miri.datamodels.miri_pixel_saturation_model import \
    saturation_reference_flags, MiriPixelSaturationModel
from miri.datamodels.tests.util import assert_products_equal


class TestMiriPixelSaturationModel(unittest.TestCase):
    
    # Test the MiriPixelSaturationModel class.
    # Most of the relevant tests are already done in MiriMeasuredImageModel.
    # Only the additional tests specific to MiriPixelSaturationModel are
    # included here.
    
    def setUp(self):
        # Create a typical pixel_saturation product.
        a1 = np.array([[1.0,1.2,1.1],[1.3,1.2,1.0],[1.1,0.8,0.9]])
        b1 = np.array([[1.,1.,1.],[2.,2.,2.],[1.,1.,1.]])
        c1 = np.array([[0,1,0],[1,0,1],[0,1,0]])
        self.dqdef = saturation_reference_flags
        self.dataproduct = MiriPixelSaturationModel(data=a1, err=b1, dq=c1,
                                              dq_def=self.dqdef)
        ...
        self.testfile = "MiriPixelSaturationModel_test.fits"
    ...
    def test_creation(self):
        ...
        # the same shape.
        emptydp = MiriPixelSaturationModel( (4,4) )
