...
An extension to the standard STScI data model for MIRI gain data, based
...
__all__ = ['MiriGainModel']


class MiriGainModel(MiriDataModel, HasData):
    """
    ...
    def on_save(self, path):
       super(MiriGainModel, self).on_save(path)
...
if __name__ == '__main__':
    print("Testing the MiriGainModel module.")
    ...
    print("Gain model with data :")
    with MiriGainModel( data=data3x3 ) as testdata:
        print(testdata)
        if PLOTTING:
            testdata.plot(description="testdata")
        if SAVE_FILES:
            testdata.save("test_gain_model1.fits", overwrite=True)
        del testdata
